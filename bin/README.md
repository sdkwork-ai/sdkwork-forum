...
`sdkwork-forum` ships the standard nine `bin/` entrypoints. Shared behavior
...
| Script | Purpose |
| --- | --- |
| `docker-image.sh` | build / push / save / load / update / inspect `registry.sdkwork.com/apps/sdkwork-forum-standalone:<version>` |
...
Declared app types: `server,h5,pc`. Default image tag comes from
